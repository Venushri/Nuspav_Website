...
// const categories = [
//   { name: 'Jeans', image: 'https://images.unsplash.com/photo-1541099649105-f69ad21f3246', link: '/shop?cat=jeans' },
//   { name: 'T-Shirts', image: 'https://images.unsplash.com/photo-1512436991641-6745cdb1723f', link: '/shop?cat=tshirts' },
//   { name: 'Dresses', image: 'https://images.unsplash.com/photo-1520975913545-89bba160d2a4', link: '/shop?cat=dresses' },
//   { name: 'Shoes', image: 'https://images.unsplash.com/photo-1528701800484-3f2b3e3b6a49', link: '/shop?cat=shoes' },
// ];
...
const Home = () => (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
        {featuredProducts.map(p => (
          <div key={p.id} className="bg-gray-100 rounded-xl shadow p-4 text-center hover:shadow-lg transition">
            ...
            <Link to={`/products/${p.id}`} className="bg-black text-white px-4 py-2 rounded hover:bg-gray-800">View</Link>
          </div>
        ))}
      </div>
    </section>
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
    {[
      {
        icon: <i className="bi bi-search-heart-fill text-3xl text-black-500"></i>,
        title: 'Browse',
        desc: 'Explore unique thrift pieces.'
      },
      {
        icon: <i className="bi bi-handbag-fill text-3xl text-black-500"></i>,
        title: 'Add to Cart',
        desc: 'Pick your favorites before they’re gone!'
      },
      {
        icon: <i className="bi bi-credit-card-fill text-3xl text-black-500"></i>,
        title: 'Checkout',
        desc: 'Easy, secure, and fast payment.'
      },
      {
        icon: <i className="bi bi-gift-fill text-3xl text-black-500"></i>,
        title: 'Enjoy',
        desc: 'Receive your thrift treasures!'
      }
    ].map((step, i) => (
      <div
        key={i}
        className="bg-[#C0C9EE]  rounded-2xl p-6 shadow-md hover:shadow-2xl hover:scale-105 transition-all duration-300 border border-gray-100 flex flex-col items-center"
      >
 ...
 <section className="w-full mt-10 px-6 md:px-16 bg-pink-50">
  <div className="flex flex-col md:flex-row items-stretch justify-between gap-10">
    ...
    <div className="md:w-1/2 flex items-center text-center md:text-left">
      <div>
        <p
          className="text-2xl text-pink-700 font-semibold flex items-center justify-center md:justify-start gap-2 mb-4"
          style={{ fontFamily: "'Dancing Script', cursive" }}
        >
          Created with love
          <i className="bi bi-balloon-heart-fill text-pink-500 text-3xl align-middle"></i>
        ...
        <h2 className="text-lg font-medium mb-4 flex items-center justify-center md:justify-start gap-2">
          ...
          <i className="bi bi-globe-americas-fill text-xl text-green-800 align-middle"></i>
        </h2>
    ...
    <div className="md:w-1/2 flex justify-end items-end">
      <img
        src={thriftImage}
        alt="Thrift Movement"
        className="max-h-[400px] w-auto object-cover shadow-lg"
      />
    </div>
  </div>
</section>
